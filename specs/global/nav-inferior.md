...
- Íconos de `lucide-react` (`Trophy` para Ranking, `User` para Perfil); ícono personalizado de pelota de tenis para Partidos; fallback a emoji si falla la carga. Usar `text-primary` para el color base de los íconos y variantes sólidas para el estado activo.
