...
## 2026-08-17 - [Stretched Card Link Focus Rings & MDS Solid Recommended Styling]
**Learning:** Full-card stretched overlay links (`absolute inset-0`) often lack visual keyboard focus outlines, making keyboard navigation through card feeds invisible to users. Adding standard high-visibility focus indicator rings (`focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 ring-offset-background`) ensures keyboard navigation is immediately clear. Furthermore, replacing translucent highlights (like `bg-primary/5`) with solid border styling enforces MDS container rules without sacrificing visual emphasis.
**Action:** Always apply `focus-visible` ring utility styles to full-card overlay links and use solid background/border styles for featured or recommended card items.

## 2026-08-18 - [Eliminating Nested Interactive Controls & MDS Solid Card Slot Displays]
**Learning:** Using `role="button"` on a container element that encloses interactive `<button>` children violates HTML/ARIA semantics and breaks keyboard navigation (causing double focus traps and screen reader confusion). Splitting the container into separate, non-nested sibling `<button>` elements inside a clean wrapper `div` provides flawless keyboard Tabbing and clean screen-reader announcements. Furthermore, replacing translucent `bg-primary/5` background highlights with solid `bg-card` and `bg-muted` enforces Minimal Design System (MDS) container rules while standardizing high-visibility `focus-visible` rings.
**Action:** Never place interactive `<button>` elements inside an element with `role="button"` or another `<button>`. Always separate secondary actions into sibling controls with explicit localized `aria-label` tags.
