...
export default async function RankingPage({ searchParams }: RankingPageProps) {
  ...
  // Get the most recent update time from the database (last user update with ranking data)
  const lastUpdate = players.length > 0
    ? new Date(Math.max(...players.map(p => p.updatedAt.getTime())))
    : new Date();
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {player.attendanceScore >= 0.9 && (
                            <ShieldCheck className={cn(
                              "h-3 w-3",
                              player.id === viewerId ? "text-primary-foreground/60" : "text-emerald-500/60"
                            )} />
                          )}
                        </div>
