...
export function CreateTurnForm({ userLevel }: CreateTurnFormProps) {
  ...
  const levelMismatch = userLevel !== formData.suggestedLevel;
  const isUserWeakerThanSuggested = userLevel > formData.suggestedLevel; // Nivel 1 es Pro, Nivel 8 es Principiante
        ...
        <div className="p-6 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <select
              ...
              onChange={(e) =>
                setFormData({ ...formData, suggestedLevel: Number(e.target.value) })
              }
              disabled={isPending}
            ...
            <p className="text-xs text-muted-foreground leading-normal">
              {levelOptions.find((opt) => Number(opt.value) === formData.suggestedLevel)?.description ??
                "Nivel de referencia aconsejado para los participantes."}
            </p>
                ...
                <div>
                  ...
                  <p className="mt-0.5">
                    {isUserWeakerThanSuggested
                      ? `Tu nivel (Nivel ${userLevel}) es menor que el sugerido para este turno. El partido podría resultar muy exigente.`
                      : `Tu nivel (Nivel ${userLevel}) es mayor que el sugerido para este turno. El partido podría resultar poco competitivo para vos.`}
                  </p>
                </div>
              </div>
            )}
